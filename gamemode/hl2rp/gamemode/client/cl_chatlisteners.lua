...
local function drawChatReceivers()
	...
	-- No one hears you
	if #receivers == 0 then
		draw.WordBox(2, x, y, DarkRP.getPhrase("hear_noone", currentConfig.text), "ChatFont", Color(0,0,0,160), Color(255,10,10,255))
		return
	-- Everyone hears you
	elseif #receivers == #player.GetAll() - 1 then
		draw.WordBox(2, x, y, DarkRP.getPhrase("hear_everyone"), "ChatFont", Color(0,0,0,160), Color(0,255,0,255))
		return
	end

	draw.WordBox(2, x, y - (#receivers * 21), DarkRP.getPhrase("hear_certain_persons", currentConfig.text), "ChatFont", Color(0,0,0,160), Color(0,255,0,255))
...
local function chatGetRecipients()
	...
	for _, ply in pairs(player.GetAll()) do
		if ply:FAdmin_GetGlobal("FAdmin_cloaked") then continue end
...
GM:AddChatReceiver("/looc", "speak in LOOC", function(ply) return LocalPlayer():GetPos():Distance(ply:GetPos()) < 250 end)
GM:AddChatReceiver(".//", "speak in LOOC", function(ply) return LocalPlayer():GetPos():Distance(ply:GetPos()) < 250 end)
