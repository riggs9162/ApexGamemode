...
function GM:CanDemote(ply, target, reason)
 if ply:Team() == TEAM_MAYOR then
 	return true
 elseif ply:Team() == TEAM_HEALTHINSPECTOR and target:Team() == TEAM_MAYOR then
 	return true
 else
 	return false
 end
...
return ply:IsAdmin() or ply:GetNWString("usergroup") == "vip" 
end
...
function GM:PlayerSpawnedProp(ply, model, ent)
	...
	if GAMEMODE.Config.proppaying and not ply:IsAdmin() then

          if inShellBeach(ply) then
		if ply:CanAfford(2) then
			GAMEMODE:Notify(ply, 0, 4, "Deducted " .. GAMEMODE.Config.currency .. "2 (lowered due to cheap Shell Beach imports)")
			ply:AddMoney(-2)
		else
			GAMEMODE:Notify(ply, 1, 4, "Need " .. GAMEMODE.Config.currency .. "2")
...
--	if not FPP.Protect.CanTool(ply, trace, tool) then
--		return false
--	end
	--print(mode)
	if (mode == "duplicator" or mode == "dynamite" or mode == "wire_explosive" or mode == "wire_simple_explosive") and not ply:IsAdmin() then return false end
	if mode == "wire_turret" then return false end
	if not ply:IsAdmin() and mode == "paint" then
		return false
	end
	if ply:GetNWString("usergroup") ~= "vip" and (not ply:IsAdmin()) then
		if (mode == "wire_expression2" or mode == "wire_spu" or mode == "wire_egp") then
			GAMEMODE:Notify(ply, 1, 4, mode.." is VIP only.")
			return false
		end
	end
...
function GM:PlayerDeath(ply, weapon, killer)
	...
	if ply:isArrested() and not GAMEMODE.Config.respawninjail  then
		...
		if not ply:IsAdmin() and not ply:IsVIP() then
...
function GM:PlayerInitialSpawn(ply)
	...
	ply:NewData()
	ply.SID = ply:UserID()
                ply:SetDarkRPVar( "ration", "no" )
                timer.Simple(780, function() if ply:IsValid() then ply:SetDarkRPVar( "ration", "yes" ) ply:notify "You can now collect your hourly ration." ply:ConCommand( "play buttons/blip1.wav" ) end end )
...
function GM:PlayerSpawn(ply)
	...
	ply:SetPos(Vector(-4208.889160, 3195.598145, 547.843750))
	...
	if not ply.Babygod then
		...
		timer.Create(ply:EntIndex() .. "babygod", 2, 0, function()
			if not IsValid(ply) or not ply.Babygod then return end
			if ply:GetPos():Distance(Vector(-4215.968750, 3197.494873, 528.031250)) < 180 then return end
			timer.Destroy(ply:EntIndex() .. "babygod")
			ply.Babygod = nil
	...
	if ply.firedWhileDead then
		ply.firedWhileDead = nil
	...
	/*spawntime = 30
	umsg.Start( "SpawntimeReset", ply )
    umsg.String( spawntime )
	umsg.End()*/
end
...
function GM:InitPostEntity()
	...
	-- Oh wait FUCK YOU FPTJE ENABLELING SHIT LIKE THAT ON DEFAULT.
...
function GM:PlayerSpawnRagdoll( ply, model )
	if ( not ply:IsAdmin() ) then
		GAMEMODE:Notify(ply, 1, 4, "Disabled until futher notice.")
		return false
	else
		return true
	end
