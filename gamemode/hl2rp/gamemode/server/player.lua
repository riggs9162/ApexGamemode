...
local function RPName(ply, args)
	if ply.LastNameChange and ply.LastNameChange > (CurTime() - 5) then
		ply:notify( DarkRP.getPhrase("have_to_wait",  math.ceil(5 - (CurTime() - ply.LastNameChange)), "/rpname"))
		return ""
	end
	if ply:IsCP() and ply:Team() != TEAM_ADMINISTRATOR and ply:Team() == TEAM_OVERWATCH and ply:Team() != TEAM_OVERWATCHELITE then
		ply:notify(  "Civil Protection units are unable to change their name.")
		return ""
	end

	if not GAMEMODE.Config.allowrpnames then
		ply:notify(  DarkRP.getPhrase("disabled", "RPname", ""))
		return ""
	end
	...
	if len > 30 then
		ply:notify( DarkRP.getPhrase("unable", "RPname", "<=30"))
		return ""
	elseif len < 3 then
		ply:notify( DarkRP.getPhrase("unable", "RPname", ">2"))
		return ""
	end

	local canChangeName = hook.Call("CanChangeRPName", GAMEMODE, ply, low)
	if canChangeName == false then
		ply:notify( DarkRP.getPhrase("unable", "RPname", ""))
		return ""
	end
	...
	local notAllowed = {'CCA', 'SeC', 'DvL', 'CmD'}

	for k in string.gmatch(args, ".") do
		if not table.HasValue(allowed, string.lower(k)) then
			ply:notify( DarkRP.getPhrase("unable", "RPname", k))
			return ""
		end
		if string.match(args, "CCA") or string.match(args, "OWA")then
			ply:notify( "Unable to set RPName as: "..args)
			return ""
		end
	end


	if ply:Team() == "TEAM_CP" or ply:Team() == "TEAM_OW" then
		ply:notify(  "CP and OWA cannot change their names.")
		return ""
	end
...
function meta:IsDispatch()
	if self:IsCP() then
--		print(self:getDarkRPVar("Rank"))
		if self:Team()==TEAM_DISPATCH then
			return true
		end
		if (self:getDarkRPVar("Division") and self:getDarkRPVar("Division") == 7 or self:getDarkRPVar("Division") == 8) or (self:getDarkRPVar("Rank") and self:getDarkRPVar("Rank") == 9) then
			return true
		end
	end
...
return false
	...
	if t ~= GAMEMODE.DefaultTeam and not self:ChangeAllowed(t) and not force then
		self:notify(DarkRP.getPhrase("unable", team.GetName(t), "banned/fired"))
		return false
	end

	if self.LastJob and jobdelay - (CurTime() - self.LastJob) >= 0 and not force then
		self:notify(DarkRP.getPhrase("have_to_wait",  math.ceil(jobdelay - (CurTime() - self.LastJob)), "/job"))
		return false
	end
	...
	if not self.DarkRPVars["Priv"..TEAM.command] and not force and not self:IsVIP() then
		...
		max < 1 and (team.NumPlayers(t) + 1) / #player.GetAll() > max) then -- fractional limit (in percentages)
			self:notify(DarkRP.getPhrase("team_limit_reached", TEAM.name))
			return false
		end
	end
...
-- Check for XP Requirement
	...
	xp = self:GetNetworkedInt( "Xp" ) or 0
	...
	if not self:isArrested() then
		DB.RetrieveSalary(self, function(amount)
			...
			if amount == 0 or not amount then
				// self:notify( DarkRP.getPhrase("payday_unemployed"))
			else
				if not self:GetPData( "bankmoney") then self:SetPData( "bankmoney", 10) end
				self:SetPData( "bankmoney", self:GetPData( "bankmoney" ) + amount )
				self:notify( DarkRP.getPhrase("payday_message", GAMEMODE.Config.currency .. amount))
			end
		end)
	else
		self:notify( DarkRP.getPhrase("payday_missed"))
	end
end
...
local function checkCCA( ply )
	if string.match(ply:Nick(), "CCA")then
		if ply:GetPData("cname") then
			DB.StoreRPName(ply, ply:GetPData("cname"))
		else

		end
		ply:SetDarkRPVar("citopt", 0)
	end
end
...
hook.Add( "PlayerSpawn", "some_unique_name", checkCCA )