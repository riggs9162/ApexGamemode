...
local function SetXP(ply, cmd, args)

	if not ply:IsSuperAdmin() then return end
	if not args[2] then ply:PrintMessage(HUD_PRINTCONSOLE, "Invalid arguments") return end
	if ply:EntIndex() ~= 0 and not ply:HasPriv("rp_commands") then
		ply:PrintMessage(2, DarkRP.getPhrase("need_admin", "rp_unarrest"))
		return
	end
	...
	if target then
		...
		if ply:EntIndex() == 0 then
			DB.Log("Console force-set the xp of "..target:SteamName() .." to "..args[2].."XP", nil, Color(0, 255, 255))
		else
			DB.Log(ply:Nick().." ("..ply:SteamID()..") set the model of "..target:SteamName() .." to "..args[2].."XP", nil, Color(0, 255, 255))
		end
	else
		if ply:EntIndex() == 0 then
			print(DarkRP.getPhrase("could_not_find", "player: "..tostring(args[1])))
...
local function SetModel(ply, cmd, args)
	...
	if target then
		...
		if ply:EntIndex() == 0 then
			DB.Log("Console force-set the model of "..target:SteamName(), nil, Color(0, 255, 255))
		else
			DB.Log(ply:Nick().." ("..ply:SteamID()..") set the model of "..target:SteamName(), nil, Color(0, 255, 255))
		end
	else
		if ply:EntIndex() == 0 then
			print(DarkRP.getPhrase("could_not_find", "player: "..tostring(args[1])))
...
local function SetArmor(ply, cmd, args)
	...
	if target then
		...
		if ply:EntIndex() == 0 then
			DB.Log("Console force-set the armor of "..target:SteamName() .." to: "..args[2], nil, Color(0, 255, 255))
		else
			DB.Log(ply:Nick().." ("..ply:SteamID()..") set the model of "..target:SteamName(), nil, Color(0, 255, 255))
		end
	else
...
concommand.Add("rp_setarmor", SetArmor)
...
local function ccVehicleSpawn(ply, cmd, args)
	if GAMEMODE.Config.adminvehicles then
		if ply:GetNWString("usergroup") == "vip" and (string.match( string.lower(args[1]), "seat" ) or string.match( string.lower(args[1]), "chair" )) then
		--	Spawn_Vehicle(ply, args[1])
		--	DB.Log(ply:Nick().." ("..ply:SteamID()..") spawned Vehicle "..args[1], nil, Color(255, 255, 0))
		elseif ply:EntIndex() ~= 0 and not ply:IsAdmin() then
...
local function ccSetJob(ply, cmd, args)
	if not args[1] then return end
	if ply:EntIndex() ~= 0 and not ply:IsAdmin() then
		ply:PrintMessage(2, DarkRP.getPhrase("need_sadmin", "rp_setjob"))
		return
	end
	...
	if target then
		...
		if ply:EntIndex() == 0 then
			...
			ply:PrintMessage(2, "Set " .. oldname .. "'s job to: " .. args[2])
			nick = ply:Nick()
		end
		target:PrintMessage(2, nick .. " set your job to: " .. args[2])
		if ply:EntIndex() == 0 then
			DB.Log("Console set "..target:SteamName().."'s job to " .. args[2], nil, Color(30, 30, 30))
		else
			DB.Log(ply:Nick().." ("..ply:SteamID()..") set "..target:SteamName().."'s job to " .. args[2], nil, Color(30, 30, 30))
		end
	else
		if ply:EntIndex() == 0 then
			print(DarkRP.getPhrase("could_not_find", "player: "..tostring(args[1])))
...
concommand.Add("rp_setjob", ccSetJob)
