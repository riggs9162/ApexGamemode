/*---------------------------------------------------------
 ...
 ---------------------------------------------------------*/
	...
	function playerMeta:isWalking()
		return vectorLength2D(self.GetVelocity(self)) > 1
	end
...
local function SetDoorOwnable(ply)
	if time then
		ply:notify( DarkRP.getPhrase("have_to_wait", "0.1", "/toggleownable"))
		return ""
	end
	time = true
	...
	if not ply:HasPriv("rp_doorManipulation") then
		ply:notify( "You need the rp_doorManipulation privilege")
		return ""
	end
	...
	if not IsValid(ent) or (not ent:IsDoor() and not ent:IsVehicle()) or ply:GetPos():Distance(ent:GetPos()) > 115 then
		ply:notify( DarkRP.getPhrase("must_be_looking_at", "vehicle/door"))
		return
	end
...
local function SetDoorGroupOwnable(ply, arg)
	if time3 then
		ply:notify( DarkRP.getPhrase("have_to_wait", "0.1", "/togglegroupownable"))
		return ""
	end
	time3 = true
	...
	if not RPExtraTeamDoors[arg] and arg ~= "" then ply:notify( "Door group does not exist!") return "" end
...
--	print(arg)
	...
	ply:notify( "Door group set successfully")
	return ""
end
...
local function SetDoorTeamOwnable(ply, arg)
	if time4 then
		ply:notify( DarkRP.getPhrase("have_to_wait", "0.1", "/toggleteamownable"))
		return ""
	end
	time4 = true
	...
	arg = tonumber(arg)
	if not RPExtraTeams[arg] and arg ~= nil then ply:notify( "Job does not exist!") return "" end
	if IsValid(ent:GetDoorOwner()) then
		ent:UnOwn(ent:GetDoorOwner())
	end
	...
	if IsValid(trace.Entity) and trace.Entity:IsOwnable() and ply:GetPos():Distance(trace.Entity:GetPos()) < 200 then
		...
		if trace.Entity:OwnedBy(ply) then
			...
			if trace.Entity:IsOwned() and not trace.Entity:AllowedToOwn(ply) then
				ply:notify( DarkRP.getPhrase("door_already_owned"))
				return ""
			end
			...
			if( !ply:CanAfford( iCost ) ) then
				ply:notify( trace.Entity:IsVehicle() and DarkRP.getPhrase("vehicle_cannot_afford") or DarkRP.getPhrase("door_cannot_afford") );
				return "";
			end
			...
			if not bVehicle and (ply.OwnedNumz or 0) >= GAMEMODE.Config.maxdoors then
				ply:notify( DarkRP.getPhrase("limit", "door"))
				return ""
			end
...
local function UnOwnAll(ply, cmd, args)
	...
	ply:notify(string.format("You have sold "..amount.." doors for " .. GAMEMODE.Config.currency .. amount * math.floor(((GAMEMODE.Config.doorcost * 0.66666666666666)+0.5)) .. "!"))
	return ""
end
...
local function RemoveDoorOwner(ply, args)
	...
	if trace.Entity.DoorData.NonOwnable then
		ply:notify( DarkRP.getPhrase("door_rem_owners_unownable"))
		return ""
	end

	if not target then
		ply:notify( DarkRP.getPhrase("could_not_find", "player: "..tostring(args)))
		return ""
	end

	if not trace.Entity:OwnedBy(ply) then
		ply:notify( DarkRP.getPhrase("do_not_own_ent"))
		return ""
	end
...
local function AddDoorOwner(ply, args)
	...
	if trace.Entity.DoorData.NonOwnable then
		ply:notify( DarkRP.getPhrase("door_add_owners_unownable"))
		return ""
	end
	...
	if trace.Entity:OwnedBy(target) or trace.Entity:AllowedToOwn(target) then
		ply:notify( DarkRP.getPhrase("rp_addowner_already_owns_door", ply:Nick()))
		return ""
	end
