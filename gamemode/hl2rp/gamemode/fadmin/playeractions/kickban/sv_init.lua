	...
	for _, target in pairs(targets) do
		...
		elseif stage == "update" then -- Update reason text
			...
			if type(target) ~= "string" and  IsValid(target) then
				...
				local nick = ply:GetName() or "console"
				...
				SaveBan(target, nil, time, Reason ~= "" and Reason, ply:GetName() or "console", ply.SteamID and ply:SteamID() or "Console") -- Again default to one hour
...
concommand.Add("getInfo", function(ply)
	-- body
end)
